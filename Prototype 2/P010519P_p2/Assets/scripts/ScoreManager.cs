...
public class ScoreManager : MonoBehaviour
{
    ...
    public float radioFadeSpeed = 2f;
    ...
    void Start()
    {
        // find player ref via tag
        GameObject playerObj = GameObject.FindGameObjectWithTag("Player");
        if (playerObj != null) player = playerObj.transform;

        // initialize dialogue text as invisible
        ...
        if (radioSource != null) radioSource.volume = 0f;
    }
    ...
    void Update()
    {
        ...
        // calculate distortion level based on overall game progress
        float scorePercent = currentScore / 50000f;
        distortionAmount = Mathf.Clamp01(scorePercent);

        // calculate multiplier based on player altitude
        ...
        // accumulate score based on horizontal velocity and current multiplier
    ...
    void CheckDialogue()
    {
        ...
        // trigger next dialogue line if score threshold is reached
    ...
    IEnumerator PlayDialogue(string message)
    {
        ...
        {
            // scale glitch frequency with world distortion
    ...
    void StartRadio()
    {
        ...
        // swap radio clip based on distortion progress
        ...
        if (radioFadeRoutine != null) StopCoroutine(radioFadeRoutine);
    ...
    IEnumerator FadeRadio(float targetVolume)
    {
        ...
        {
            radioSource.volume = Mathf.MoveTowards(radioSource.volume, targetVolume, radioFadeSpeed * Time.deltaTime);
            yield return null;
        }

        if (targetVolume == 0f) radioSource.Stop();
    }
    ...
    {
        // change multiplier color based on current tier
